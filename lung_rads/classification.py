...
def determine_attenuation(row):
    if row['O nódulo tem atenuação sólida ou em partes moles?']:
        return 'Sólido'
    #elif row['O nódulo tem atenuação semi-sólida ou parcialmente sólida?']:
    #    return 'Parcialmente Sólido'
    elif row['O nódulo tem atenuação em vidro fosco?']:
        return 'Vidro Fosco'
    return 'Desconhecido'

# Função para determinar os bordos (espiculado ou não)
def determine_edges(row):
    return 'Espiculada' if row['O nódulo tem borda espiculada ou irregular ou mal definida?'] else 'Não Espiculada'
