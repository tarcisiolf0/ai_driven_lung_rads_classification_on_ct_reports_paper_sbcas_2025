...
if __name__ == '__main__':
    ...
    train_df = pd.read_csv("doc_similarity/data/train.csv")
    test_df = pd.read_csv("doc_similarity/data/test.csv")
    ...
    for i, embedding_test in enumerate(embeddings_test):
        ...
        resultados['report_idx'].append(test_df['Laudo'][i])
        #resultados['1_similar'].append(train_df['Laudo'][results[0]])
        resultados['five_most_similars'].append(train_df['Laudo'][results[:5]].tolist())
    ...
    df_resultados.to_csv('doc_similarity/data/similarity_results.csv', index=False)
