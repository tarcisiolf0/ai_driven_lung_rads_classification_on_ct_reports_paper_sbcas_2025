...
def train_test_split_csv_files(filename, indexes, test_size, random_seed):
    df = read_odf_file(filename)

    df_total = df.loc[df["Laudo"].isin(indexes)]
    train_df, test_df = train_test_split(df_total, test_size=test_size, random_state=random_seed, stratify=df_total['Lung-RADS'])

    train_distribution_strat = train_df['Lung-RADS'].value_counts(normalize=True) * 100
    test_distribution_strat = test_df['Lung-RADS'].value_counts(normalize=True) * 100
    ...
    #print(train_df['Lung-RADS'].value_counts())
    #print()
    #print(test_df['Lung-RADS'].value_counts())

    train_df = train_df.rename(columns={"O nódulo é sólido ou em partes moles?" : "O nódulo tem atenuação sólida ou em partes moles?",
                                        "O nódulo tem densidade semissólida ou parcialmente sólida?" : "O nódulo tem atenuação semi-sólida ou parcialmente sólida?",
                                        "O nódulo tem densidade em vidro fosco?" : "O nódulo tem atenuação em vidro fosco?",
                                        "O nódulo tem borda espiculada e/ou mal definida?" : "O nódulo tem borda espiculada ou irregular ou mal definida?",
                             })
    
    test_df = test_df.rename(columns={"O nódulo é sólido ou em partes moles?" : "O nódulo tem atenuação sólida ou em partes moles?",
                                        "O nódulo tem densidade semissólida ou parcialmente sólida?" : "O nódulo tem atenuação semi-sólida ou parcialmente sólida?",
                                        "O nódulo tem densidade em vidro fosco?" : "O nódulo tem atenuação em vidro fosco?",
                                        "O nódulo tem borda espiculada e/ou mal definida?" : "O nódulo tem borda espiculada ou irregular ou mal definida?",
                             })
    train_df.to_csv("doc_similarity/data/train.csv", index=False)
    test_df.to_csv("doc_similarity/data/test.csv", index=False)
...
def csv_to_json(csv_filename, output_filename):
    ...
    """
        df = pd.read_csv(csv_filename, usecols=['Laudo',
                                     'Texto', 
                                     'O nódulo tem atenuação sólida ou em partes moles?', 
                                     'O nódulo tem atenuação semi-sólida ou parcialmente sólida?', 
                                     'O nódulo tem atenuação em vidro fosco?',
                                     'O nódulo tem borda espiculada ou irregular ou mal definida?', 
    ...
    for idx, row in df.iterrows():
        ...
        "O nódulo tem atenuação sólida ou em partes moles?" : row["O nódulo tem atenuação sólida ou em partes moles?"],
        "O nódulo tem atenuação semi-sólida ou parcialmente sólida?" : row["O nódulo tem atenuação semi-sólida ou parcialmente sólida?"],
        "O nódulo tem atenuação em vidro fosco?" : row["O nódulo tem atenuação em vidro fosco?"],
        "O nódulo tem borda espiculada ou irregular ou mal definida?" : row["O nódulo tem borda espiculada ou irregular ou mal definida?"],
...
if __name__ == '__main__':
    ...
    #train_test_split_csv_files(odf_filename, indexes_usable, test_size=0.30, random_seed=42)
